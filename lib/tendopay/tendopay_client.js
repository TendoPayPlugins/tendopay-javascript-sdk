...
const CancelTransactionResponse = require('./tendopay_cancel_transaction_response').CancelTransactionResponse;
...
class Client {
  ...
  async cancelTransaction ({cancelRequest}) {
    let params = {
      [constants.params.transactionNumberParam]: cancelRequest.transactionNumber
    };

    let response = await this._httpClient.get(constants.http.cancellationEndpointURI, {
      params: {
        ...params,
        tendopay_hash: utils.hash(params)
      },
